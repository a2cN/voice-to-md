 ...
 * メインロジック
 */
function processVoiceMemosCombined(): void {
  ...
  try {
    ...
    if (!titleMatch || !bodyMatch) throw new Error('レスポンス形式のパースに失敗しました。');
    ...
    const errorMsg = `❌ エラー発生: ${(e as Error).message}`;
